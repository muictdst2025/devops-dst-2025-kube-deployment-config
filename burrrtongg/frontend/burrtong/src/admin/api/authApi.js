...
export const signupAdmin = async (email, password) => {
  ...
  if (!response.ok) {
    console.error("Admin Signup API Error Status:", response.status);
    console.error("Admin Signup API Error Body:", responseBodyText);
  ...
  try {
    ...
    console.error("Admin Signup API Success, but response was not valid JSON:", responseBodyText);
    throw new Error('Admin signup successful, but response was not valid JSON.');
  }
};